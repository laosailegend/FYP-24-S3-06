...
//retrieving schedules based on userid
app.get('/schedules', (req, res) => {
    ...
    let query = `
        SELECT 
            ...
            schedules.end_time,
            schedules.is_weekend,
            schedules.is_public_holiday,
            schedules.salary, 
    ...
    if (userId) {
        query += ' WHERE schedules.userid = ?';
        params.push(userId); // Add userId to the parameters array
    }

    if (shiftDate) {
        query += userId ? ' AND schedules.shift_date = ?' : ' WHERE schedules.shift_date = ?';
        params.push(shiftDate); // Add shiftDate to the parameters array
    }
...
app.post('/payroll', async (req, res) => {
  ...
  app.get('/user/schedules/:userId', (req, res) => {
